...
def prepare_crown_pointcloud(jaw_point_folder, mesh_name, jaw_size=2048, average_sp=True):
...
def load_occ_points(occ_path) -> torch.FloatTensor:
    with np.load(occ_path) as data:
        vol_points = data['vol_points']
        vol_label = data['vol_label']
    vol_points = torch.from_numpy(vol_points).unsqueeze(0).cuda()
    vol_label = torch.from_numpy(vol_label).float().unsqueeze(0).cuda()
    return vol_points, vol_label
...
def crown_mesh_generation(args, model:crown_diffusion.Crown_denoiser, diffusion_system:crown_diffusion.Crown_diffusion_system, cfg: dict, 
    ...
    occ_path = os.path.join(os.path.dirname(args.txt_path), 'crown_occ')
    ious = []
    for i, mesh_name in enumerate(mesh_list):
        ...
        vol_points, vol_label = load_occ_points(os.path.join(occ_path, mesh_name))

        crown_cate = torch.LongTensor([tooth_mapping[mesh_name.split('_')[-1][:-4]]]).cuda()
        crown_cate = crown_cate.unsqueeze(0)
        
        # Generate the crown
        decoded_latents = diffusion_system.sample(model, jaw_pointcloud, 1, 500, categories=crown_cate)[0]
        iou = diffusion_system.cal_iou(decoded_latents, bounds=box_v, octree_depth=octree_depth, 
                                               query_points=vol_points, occ_labels=vol_label)
        ious.append(iou)
        
        print(f'-----------------------------------------------------------------------------')
        print(f'>>> Mesh {mesh_name} has iou: {iou}')
        print(f'-----------------------------------------------------------------------------') 
