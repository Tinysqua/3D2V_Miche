...
class FPSCrossAttentionEncoder(nn.Module):
...
class ShapeAsLatentPerceiver(ShapeAsLatentModule):
    def __init__(self, *,
                 device: Optional[torch.device],
                 dtype: Optional[torch.dtype],
                 num_latents: int,   # 相当于你从全体点中选择了多少个点作为代表数据，默认512
                 point_feats: int = 0, # 除了点云信息额外信息的量，默认0
                 embed_dim: int = 0,   # kl变量的dimension（包括mean, var），默认64
                 num_freqs: int = 8,   # 点云嵌入相关，默认8
                 include_pi: bool = True, 
                 width: int,           # surface点云嵌入之后的dimension长度，默认768
                 heads: int,           # 多少个头的注意力，默认12
                 num_encoder_layers: int,  # points query在cross atten surface点云之后经过一个多少层的transformer
                 num_decoder_layers: int,  # 在查询geometry信息之前经过一个多少层的transformer
                 init_scale: float = 0.25,
                 qkv_bias: bool = True,
                 flash: bool = False,
                 use_ln_post: bool = False,
...
class FPSShapePerceiver(ShapeAsLatentModule):
    def __init__(self, *,
                 device: Optional[torch.device],
                 dtype: Optional[torch.dtype],
                 num_latents: int,   # 相当于你从全体点中选择了多少个点作为代表数据，默认512
        ...
        init_scale = init_scale * math.sqrt(1.0 / width)
        self.encoder = FPSCrossAttentionEncoder(
            device=device,
            dtype=dtype,
            fourier_embedder=self.fourier_embedder,
            num_latents=num_latents,
