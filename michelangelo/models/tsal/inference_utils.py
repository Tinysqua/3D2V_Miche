...
def extract_geometry(geometric_func: Callable,
    ...
    Args:
        ...
        device:
        bounds: 
        octree_depth: always 7 -> 2^7 == 128 resolution
        batch_size: always 1
        num_chunks: Split the query points to how many chunks
        disable: Whether disable the display of tqdm
    ...
    # Will run one time actually in our project
...
@torch.no_grad()
def geometry_iou(geometric_func: Callable,
                device: torch.device,
                batch_size: int = 1,
                bounds: Union[Tuple[float], List[float], float] = (-1.25, -1.25, -1.25, 1.25, 1.25, 1.25),
                octree_depth: int = 7,
    ...
    logits = geometric_func(query_points)

    threshold = 0
    pred = torch.zeros_like(logits)
    pred[logits>=threshold] = 1

    accuracy = (pred==occ_labels).float().sum(dim=1) / occ_labels.shape[1]
    accuracy = accuracy.mean()
    intersection = (pred * occ_labels).sum(dim=1)
    union = (pred + occ_labels).gt(0).sum(dim=1)
